import os
import sqlalchemy
import psycopg2
import json
from sqlalchemy.orm import sessionmaker
from models import create_tables, drop_tables, Publisher, Book, Shop, Stock, Sale
from pprint import pprint

BASE_PATH = os.getcwd()
DATA_DIR = 'fixtures'
DATA = 'tests_data.json'

with open(os.path.join(BASE_PATH, DATA_DIR, DATA), 'r') as f:
    file_content = f.read()
    templates = json.loads(file_content)

# DSN = 'используемая БД :// логин ДБ : пароль ДБ @ имя сервер : порт сервера (5432) / название ДБ'
DSN = 'postgresql://postgres:Ingrad2022@localhost:5432/sale-book'
engine = sqlalchemy.create_engine(DSN)

# create_tables(engine)
# drop_tables(engine)

Session = sessionmaker(bind=engine)
session = Session()

# Создание экземляров классов из models для добавления значений
# session.add( какое то знаение ) или
# session.add_all( [ список значений ]
# session.commit - для сохранения данных в БД


add_list = []
for element in templates:
    if element['model'] == 'publisher':
        name = element['fields']['name']
        publisher = Publisher(name=name)
        add_list.append(publisher)
    elif element['model'] == 'book':
        title = element['fields']['title']
        id_publisher = element['fields']['id_publisher']
        book = Book(title=title, id_publisher=id_publisher)
        add_list.append(book)
    elif element['model'] == 'shop':
        name = element['fields']['name']
        shop = Shop(name=name)
        add_list.append(shop)
    elif element['model'] == 'stock':
        id_shop = element['fields']['id_shop']
        id_book = element['fields']['id_book']
        count = element['fields']['count']
        stock = Stock(id_shop=id_shop, id_book=id_book, count=count)
        add_list.append(stock)
    elif element['model'] == 'sale':
        price = element['fields']['price']
        date_sale = element['fields']['date_sale']
        count = element['fields']['count']
        id_stock = element['fields']['id_stock']
        sale = Sale(price=price, date_sale=date_sale, count=count, id_stock=id_stock)
        add_list.append(sale)
# pprint(add_list)
session.add_all(add_list)
session.commit()

# Извлечение данных
# session.query ( таблица ).all() - всех данных

# Извлечение определенных данных
# session.query ( таблица ).filter( условие фильтрации ).all() - всех данных

# Объединение таблиц
# session.query( таблица ).join( таблица . свойство relationship в таблице связ с первой ).all

# Вложенные подзапросы
# subq = session.query( таблица ).filter( условие фильтрации ).subquery()
# session.query( таблица ).join(subq, условие объединения table.id == subq.c.id из таблицы в подзапросе)

# Обновление данных
# session.query( таблица ).filter( условие фильтрации ).update({"атрибут": "значение"})
# session.commit()
# с удалением аналогично
session.close()

